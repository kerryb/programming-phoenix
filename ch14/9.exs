defmodule CatCounter do
  ...
  defp count_cats filename do
    {:ok, text} = File.read(filename)
    Regex.scan(~r/cat/i, text) |> Enum.count
  end
end
...
to_process = Enum.map files, fn f -> Path.join(dir, f) end

Enum.each 1..10, fn num_processes ->
  {time, result} = :timer.tc(
    Scheduler, :run,
    [num_processes, CatCounter, :count, to_process]
  )
